...
theme: /
    ...
    state: TellWeather
        ...
        if: $temp.response.isOk
            random:
                a: У меня получилось уточнить: на {{ new Intl.DateTimeFormat("ru", {dateStyle: "long"}).format($session.userDate) }} в городе {{$session.userCity}} температура воздуха составит {{Math.floor($temp.response.data.main.temp)}} °C градусов по Цельсию.
                a: Смог узнать для вас прогноз: на {{ new Intl.DateTimeFormat("ru", {dateStyle: "long"}).format($session.userDate) }} в городе {{$session.userCity}} будет {{Math.floor($temp.response.data.main.temp)}} °C градусов по Цельсию.
