...
theme: /
    ...
    state: HowCanIHelpYou
        ...
        if: $request.channelType === "telegram"
            inlineButtons:
                ...
                { text: "Оформить заявку на подбор тура", callback_data: "tour" }
        else:
            buttons:
    ...
    state: TellWeather
        ...
        if: $temp.response.isOk
            random:
                a: У меня получилось уточнить: на {{$session.userDate.toLocaleDateString("ru-RU")}} в {{$session.userCity}} температура воздуха составит {{Math.floor($temp.response.data.main.temp)}} °C.
                a: Смог узнать для вас прогноз: на {{$session.userDate.toLocaleDateString("ru-RU")}} в {{$session.userCity}} будет {{Math.floor($temp.response.data.main.temp)}} °C.
        else:
    ...
    state: SomethingElse  
        ...
        if: $request.channelType === "telegram"
            inlineButtons:
                { text: "Узнать прогноз погоды", callback_data: "прогноз погоды" }
                { text: "Оформить заявку на подбор тура", callback_data: "подбор тура" }
        else:
            buttons:
                "Узнать прогноз погоды" -> /WeatherForecast
                "Оформить заявку на подбор тура" -> /OfferTour
