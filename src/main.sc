...
theme: /
    ...
    state: TellWeather
        ...
        if: $temp.response.isOk
            random:
                a: У меня получилось уточнить: на {{$session.userDate.toLocaleDateString("ru-RU", {year: "numeric", month: "long", day: "numeric"}) }} в {{$session.userCity}} температура воздуха составит {{Math.floor($temp.response.data.main.temp)}} °C градусов по Цельсию.
