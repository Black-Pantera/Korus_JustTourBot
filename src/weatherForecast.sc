...
theme: /
    ...
    state: WeatherForecast
        ...
        q!: * ~погода * [будет|ожидается|намечается|следует ожидать|прогнозируется|ждать] * {[@duckling.date] * [в] * [$City] } *
        q!: * (будет|ожидается|намечается|следует ожидать|прогнозируется) * [@duckling.date] * { [в] * $City * (дождь|солнечно|пасмурно|снег|туман) } *
        intent: /weather
        script:
